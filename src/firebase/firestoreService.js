...
export async function openPackToBase(payload) {
  const fromProductId = String(payload?.fromProductId || '');
  const toProductId = String(payload?.toProductId || '');
  const fromQty = Number(payload?.fromQty || 0);
  const basePerPack = Number(payload?.basePerPack || 0);

  if (!fromProductId || !toProductId) {
    throw new Error('Please select both source and target products.');
  }
  if (fromProductId === toProductId) {
    throw new Error('Source and target products must be different.');
  }
  if (!Number.isFinite(fromQty) || fromQty <= 0) {
    throw new Error('Please enter a valid quantity to open.');
  }
  if (!Number.isFinite(basePerPack) || basePerPack <= 0) {
    throw new Error('Please enter a valid conversion quantity.');
  }

  const toQty = Number((fromQty * basePerPack).toFixed(4));

  const fromRef = doc(db, 'products', fromProductId);
  const toRef = doc(db, 'products', toProductId);
  ...
  await runTransaction(db, async (txn) => {
    ...
    const fromStock = Number(fromSnap.data().stock || 0);
    if (fromStock < fromQty) {
      throw new Error('Insufficient source stock to open packaging.');
    }

    const toStock = Number(toSnap.data().stock || 0);
    txn.update(fromRef, { stock: Number((fromStock - fromQty).toFixed(4)) });
    txn.update(toRef, { stock: Number((toStock + toQty).toFixed(4)) });
  });

  const [fromAfter, toAfter] = await Promise.all([getDoc(fromRef), getDoc(toRef)]);
  const fromData = fromAfter.data() || {};
  const toData = toAfter.data() || {};

  const movementBase = {
    type: 'open-pack',
    date: payload?.date || '',
    time: payload?.time || '',
    note: payload?.note || 'Open packaging',
    inputBy: payload?.inputBy || '',
  };
  ...
  const outPayload = {
    ...
    product: fromData.name || payload?.fromProductName || fromProductId,
    qty: Number(fromQty),
    unit: fromData.baseUnit || fromData.unit || '',
    relatedProductId: toProductId,
    relatedProduct: toData.name || payload?.toProductName || toProductId,
  };
  ...
  const inPayload = {
    ...
    product: toData.name || payload?.toProductName || toProductId,
    qty: Number(toQty),
    unit: toData.baseUnit || toData.unit || '',
    relatedProductId: fromProductId,
    relatedProduct: fromData.name || payload?.fromProductName || fromProductId,
    fromQty: Number(fromQty),
    basePerPack: Number(basePerPack),
  };

  const [outRef, inRef] = await Promise.all([
    addDoc(col('stockMovements'), { ...outPayload, createdAt: serverTimestamp() }),
    addDoc(col('stockMovements'), { ...inPayload, createdAt: serverTimestamp() }),
  ]);

  return {
    updatedProducts: [
      { id: fromProductId, ...serialize(fromData) },
      { id: toProductId, ...serialize(toData) },
    ],
    movements: [
      { id: outRef.id, ...outPayload },
      { id: inRef.id, ...inPayload },
    ],
  };
}
